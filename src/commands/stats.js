module.exports = {
    ...
    execute(self, msg){
        ...
        class Stats{
            ...
            async getStats(){
                await self.db.getStats(msg.guild.id, "all", async (result) => {
                    if(result.length === 0){
            ...
            async fetchAllMessages(){
                ...
                self.send(msg, `Done, took ${(Date.now() - this.started) / 1000}seconds to fetch ${this.requests/10}k messages.`);
            ...
            updateDatabase(){
                ...
                self.db.executeStatement("INSERT INTO stats_" + msg.guild.id + " (id, value, type) VALUES ?", values, (result) => {
                    console.log(result);
                });
            }
        }
